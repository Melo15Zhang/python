...
"""命令行火车票查看器 Usage: tickets [-gdtkz] <from> <to> <date>"""
...
class TrainsCollection:
    header = '车次  始发/终点  发车/到达    历时  商务/特等 一等座  二等座  高级软卧  软卧  硬卧  硬座  无座'.split()
        ...
        for raw_train in self.available_trains:
            ...
            if not self.options or initial in self.options:
                train = [
                    ...
                    duration,
                    raw_train_list[-5] if raw_train_list[-5] else '--',  # 商务座/特等座
...
def cli():
    ...
    from_station = stationjs.get(arguments['<from>'])
    to_station = stationjs.get(arguments['<to>'])
    date = arguments['<date>']
